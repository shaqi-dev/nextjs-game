...
export default function Home() {
  ...
  const backgroundImage =
    gameMode === GameMode.START ? backgrounds.startScreen : backgrounds.game[gameStyle];

  const handleChangeItemsCountIdx = useCallback((value: number) => setItemsCountIdx(value), []);

  const handleChangeValuesIdx = useCallback((value: number) => setValuesIdx(value), []);

  const handleChangeOrder = useCallback((value: GameOrder) => setGameOrder(value), []);
